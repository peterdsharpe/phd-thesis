...
\begin{itemize}
    ...
    \begin{enumerate}
        \item As alluded to in Section \ref{sec:traceability}, the traceability requirement of code transformations imposes more restrictions on how the user can write numerical code, compared to a black-box optimization or analytic-gradients approach. Most fundamentally, the user's numerical code must be intercepted via a tracing-compatible numerics library, which must be provided as part of the code transformations framework. This naturally creates a tension against usability—introducing too much special syntax within this numerics library results in a steep learning curve. The implication here is that keeping the learning curve of framework syntax becomes critically important, if the usability benefits depicted in Table \ref{tab:paradigm_comparison} are to be realized.
        \item Beyond just the \emph{syntax} of a user's numerical code, a code transformations paradigm also favors a certain \emph{style} of numerical code. In particular, \emph{functional} coding styles that avoid side effects and mutable state are more easily traced and transformed. This is because a computational graph for code with in-place mutation tends to lead to the creation of redundant memory copies, lest information about intermediate states be lost\footnote{Bradbury et al. discuss methods for mitigating this by compiled graph transformations, though this behavior still tends to be memory-inefficient in practice \cite{jax}.}. We argue the workflow of engineers working on physics-based design optimization problems naturally tends to lead to \emph{procedural} coding styles, which creates another source of tension in framework development.
    \end{enumerate}
\end{itemize}

Notably, the challenges of code transformations are strongly coupled to syntax and interface choices during framework development, which are often underemphasized in the literature. In the following sections, we discuss these considerations in more detail, and propose some strategies for addressing them.
