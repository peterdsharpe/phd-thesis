...
\begin{enumerate}
    \item \textbf{GPkit (cvxopt)}, where the problem is solved using GPkit's default convex optimization solver, CVXOPT \cite{cvxopt}. Notably, CVXOPT is free and open-source, so this case allows an ``open-source to open-source'' comparison to be drawn.
...
To demonstrate this, we implement a benchmark problem from the user documentation of OpenMDAO, which is a popular MDO framework that supports analytic gradients.
...
%Like most framework architecture decisions, this comes down to tradeoffs
...
\subsection{Miscellaenous} % TODO fix title
