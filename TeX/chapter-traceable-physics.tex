...
This chapter contributes computational implementations of several key physics models for aircraft design that are traceable within AeroSandbox, the design optimization framework introduced in Section \ref{sec:aerosandbox}. The broad motivation for this contribution stems from the observation that ``ease-of-model-implementation'' has historically proven to be one of the most important factors for determining whether an MDO paradigm can achieve use in industry. To that end, the goals of this contribution are to:
...
To achieve these goals, this thesis contributes the traceable implementations of the analyses given in Table \ref{tab:models_to_contribute}. Several other analyses, including structural and propulsive models, were also implemented in the AeroSandbox framework, but are not described in this thesis document for brevity; the interested reader is directed to the AeroSandbox documentation for more information \cite{asb_github, sharpe_aerosandbox_2021}. The set of analyses selected for inclusion in this chapter was deliberately chosen to be mathematically diverse, spanning a wide range of common code patterns in scientific computing. The types of attributes that each analysis is intended to stress-test are given in the right-most column of Table \ref{tab:models_to_contribute}.
...
The aerodynamic operating point is at an angle of attack of $5\degree$ (measured at the fixed wing root), with a freestream velocity high enough to produce interesting aeroelastic effects.
...
\begin{figure}[H]
    ...
    \caption{Three-view of the glider aircraft geometry used in the AeroSandbox aerodynamics cross-validation study.}
...
\begin{equation}
    ...
    \left(
    \frac
    {\text{Time-averaged altitude (MSL)}}
    {({30\ \rm feet})}
    \right) +
    10^{-3} \cdot w
\end{equation}
